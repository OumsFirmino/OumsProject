...
void cGVcycle(double **xo,double **b,double **a, int **ja, int **ia, double **au, int **jau, int **iau, double **al,int **jal,int **ial,int m, int n,int nnz_tr, int LEVEL , int *count,int itt,FILE *file,double tau){
    ...
    dm_prev = malloc(n*sizeof(double));
    for(int i = 0; i < n ; i++){
    ...
    int count_pre = 0;
    conjugateGradient(&dm_prev,&r_prev,b,xo,a,ja,ia,&inval,jal,ial,n,&count_pre,2);
    ...
    // Matrices de coarse-grid;
                                ...
                                int mc = (m-1)/2 +1;
                                    ...
                                    if(*count +1 < LEVEL){
                                        ...
                                        cGVcycle(&uoc,&rc,&ac,&jac,&iac,&auc,&jauc,&iauc,&alc,&jalc,&ialc,mc,nc,nnz_tr,LEVEL,&count2,itt,file,tau);
                                    ...
                                    conjugateGradient(&dm_prev,&r_prev,b,xo,a,ja,ia,&inval,jal,ial,n,&count_post,2);
                                    residu(b,a,ja,ia,xo,&r,n);
...
free(ar);free(jr);free(ir);free(r_prev);free(dm_prev);
