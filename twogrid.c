...
void  twogrid(double **xo,double **b,double **a, int **ja, int **ia, double **au, int **jau, int **iau, double **al,int **jal,int **ial,int m, int n,int nnz_tr ,int itt,FILE *file,double tau){
...
func = &rho;
   ...
   double res2 = norme(&r,n)/norme(b,n);
    ...
    double *ec ;
    ec = malloc(nc*sizeof(double));
    for(int i = 0; i < nc; i++){
        ec[i] = 0;
    }
    if(solve_umfpack(nc,iac,jac,ac,rc,ec)){}
...
free(ar);free(jr);free(ir);
 free(inval);free(invau); free(ap);free(jp);free(ip);free(rc);free(ac);free(bc);free(auc);free(alc);free(iac);free(jac);free(iauc);free(jauc);free(ialc);free(jalc);free(r);free(ec);free(ep);
}
